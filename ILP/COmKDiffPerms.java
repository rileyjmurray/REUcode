...
public class COmKDiffPerms {
	...
	public COmKDiffPerms(double[][] procTimes, double[] weight, int[] serversArg) {
		p = procTimes;
		w = weight;
		n = p.length;
		m = p[0].length;
	...
	public void buildConstraints() throws GRBException {
		...
		/*
		 * Constraints that do NOT sum over any set of indicies
		 */
	...
	public void solveIP() throws GRBException {
		...
		if (status.equals(GRB.Status.OPTIMAL)) {
			...
			String objOut = String.format("Total weighted completion time:   %.2f", model.get(GRB.DoubleAttr.ObjVal));
